...
public class ColorFinder : MonoBehaviour {
    ...
    // "Injected" objects.
    private GameObject _videoPanelUI;
    private Renderer   _videoPanelUIRenderer;
    ...
    private void Start() {
        ...
        _videoPanelUIRenderer          = _videoPanelUI.GetComponent<Renderer>();
    ...
    private void OnFrameSampleAcquired(VideoCaptureSample sample) {
        ...
        if (sample.TryGetCameraToWorldMatrix(out cameraToWorldMatrixAsFloat) == false) {
            Debug.Log("Failed to get camera to world");
            return;
        }
        ...
        // We will try to change a colour
        // BGRA

        Color c = new Color32(0, 0, 0, 0);
        Color32 color = new Color32(0, 0, 0, 0);

        for (int i = 0; i < _latestImageBytes.Length; i += 4)
        {
            byte b = _latestImageBytes[i];
            byte g = _latestImageBytes[i + 1];
            byte r = _latestImageBytes[i + 2];
            byte a = _latestImageBytes[i + 3];
            ...
            Debug.Log("Ancient value " + c.b + " " + c.g + " " + c.r + " " + c.a);
            ...
            if (h >= 50 && h <= 63)
            {
                ...
                Debug.Log("New value " + color.b + " " + color.g + " " + color.r + " " + color.a);

                _latestImageBytes[i]     = color.b;
                _latestImageBytes[i + 1] = color.g;
                _latestImageBytes[i + 2] = color.r;
                _latestImageBytes[i + 3] = color.a;
            }
        }
